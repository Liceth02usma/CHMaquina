...
class process(object):
    def __init__(self, array, ins_num, id, ins,random_num,tiempo=0):
        self.ins = ins
        self.final = ins + ins_num - 2
        self.id = id
        self.ins_num = ins_num 
        self.prioridad = random_num
...
def obtener_numero_unico():
    global procesos
    lista =[]
    for i in procesos:
        lista.append(i.prioridad)
    print(lista,"Prioridad -> Procesos")
    numero_aleatorio = randint(0, 99)  
    
    while numero_aleatorio in lista:
        numero_aleatorio = randint(0, 99)  # Genera otro número si el anterior está en la lista
    lista.append(numero_aleatorio)
    
    return numero_aleatorio   


def create_process(array, ins):
    global procesos
    global expropiativo
    global programas
    random_num = obtener_numero_unico()
    if len(procesos) >0:
        tiempo_llegada = (procesos[-1].tiempo_llegada + procesos[-1].ins_num)/4
        procesos.append(process(array,len(array),programas,ins,random_num,tiempo_llegada))
        expropiativo.append(process(array,len(array),programas,ins,random_num,tiempo_llegada))
    else:
        procesos.append(process(array,len(array),programas,ins,random_num))
        expropiativo.append(process(array,len(array),programas,ins,random_num))
...
#________________________ FUNCIONES_______________________
...
def proceso_instruccion(i,k,acumulador,inicio):
    global principal
...
def correr_archivo():
    valor = simpledialog.askstring("Ingresar algoritmo de planificacion de procesos:", "Ingrese un valor: \n 1. FCFS\n 2. Round Robin\n 3. SJF Expropiativo\n 4. SJF No expropiativo\n 5. Por prioridad Expropiativo\n 6. Por prioridad No expropiativo ")
    ...
    if lenght(my_array) == 1 + valor_kernel:  # evalua si no se ha cargado ningun programa
        ...
        tiempo_llegada = 0  # Tiempo de llegada inicial
        print(basura,"basura")
        while len(procesos) > 0 or len(lista_espera) > 0:
            if len(procesos) != 0:
                lista_espera.insert(0,procesos.pop(0))
            for j in range(len(lista_espera)): # recorre de 0 a la cantidad de programas que hayan
                ...
                inicio = lista_espera[j].ins
                
                acumulador =  lista_espera[j].acumulador
                if valor == "1":  # FCFS
                    i = lista_espera[j].id
                    ...
                    if principal[i]["contador_errores"] == 0:
                        ...
                        while inicio <= final:
                            k = my_array[inicio]
                            acumulador,inicio = proceso_instruccion(i, k, acumulador, inicio)
                            my_array[0] = acumulador
                            inicio += 1
                        basura.append(lista_espera[j])
                        lista_espera.remove(lista_espera[j])
                    else:
                        ...
                        basura.append(lista_espera[j])
                        lista_espera.remove(lista_espera[j])
                elif valor == "2":  # Round Robin
                    i = lista_espera[j].id
                    nombre_archivo = tk.Label(ventana, text= f'{principal[i]["nombre"]}.ch', font=("Arial", 9,"bold"), fg="white", bg="#1D09F3")
                    nombre_archivo.place(y="306", x="660")
                    if principal[i]["contador_errores"] == 0:
                        for w in range(min(quantum, lista_espera[j].ins_num)):
                            k = my_array[inicio]
                            acumulador,inicio = proceso_instruccion(i, k, acumulador, inicio)
                            my_array[0] = acumulador
                            inicio += 1
                        lista_espera[j].ins = inicio
                        lista_espera[j].acumulador = acumulador
                        if lista_espera[j].final - lista_espera[j].ins + 1 <= 0:
                            if lista_espera[j] in lista_espera:
                                basura.append(lista_espera[j])
                                lista_espera.remove(lista_espera[j])
                                break
                        else:
                            lista_espera[j].ins_num = lista_espera[j].final - lista_espera[j].ins + 1
                            obj = lista_espera[j]
                            lista_espera.remove(obj)
                            lista_espera.append(obj)  
                    else:
                        principal[i]["string_errores"]  += f"Lo siento pero no puedo cargar nada ya que hay un problema en declaracion de variables\n" 
                        cuadro_texto.insert(tk.END,f'{principal[i]["string_errores"]}')   
                        cuadro_texto.insert(tk.END,f'Ha terminado la ejecucion del programa')  
                        basura.append(lista_espera[j])
                        lista_espera.remove(lista_espera[j])
                elif valor == "3":  # SJF Expropiativo
                    if cont2 == 0:
                        lista_espera = expropiativo 
                        cont2 += 1
                    print(lista_espera, expropiativo)
                    # Ordenar la lista de espera por tiempo de ejecución restante
                    lista_espera.sort(key=lambda x: (x.tiempo_llegada))
                    # Ejecutar el programa con menor tiempo de ejecución restante
                    # Ejecutar el programa con el menor tiempo de ejecución restante
                    j=0
                    i = lista_espera[j].id
                    nombre_archivo = tk.Label(ventana, text= f'{principal[i]["nombre"]}.ch', font=("Arial", 9,"bold"), fg="white", bg="#1D09F3")
                    nombre_archivo.place(y="306", x="660")
                    if principal[i]["contador_errores"] == 0:
                        ...
                        # Actualizar el programa en la lista de espera
                        lista_espera[j].ins = inicio
                        lista_espera[j].acumulador = acumulador
                        ...
                        basura.append(lista_espera[j])
                        lista_espera.remove(lista_espera[j])
                elif valor =="4": # SJF No expropiativo
                    # Ordenar la lista de espera por tiempo de ejecución restante
                    lista_espera.sort(key=lambda proceso: proceso.tiempo_llegada) 
                    ...
                    while lista_espera:
                        ...
                        if principal[i]["contador_errores"] == 0:
                            ...
                            # Actualizar el programa en la lista de espera
                            lista_espera[0].ins = inicio
                            lista_espera[0].acumulador = acumulador

                            # Verificar si el programa ha terminado
                            if lista_espera[0].final - lista_espera[0].ins + 1 <= 0:
                                if lista_espera[0] in lista_espera:
                                    basura.append(lista_espera[0])
                                    lista_espera.remove(lista_espera[0])
                            else:
                                lista_espera[0].ins_num = lista_espera[0].final - lista_espera[0].ins + 1

                                # Ordenar la lista de espera nuevamente por tiempo de ejecución restante
                            lista_espera.sort(key=lambda proceso: proceso.tiempo_llegada)
                        else:
                            principal[i]["string_errores"]  += f"Lo siento pero no puedo cargar nada ya que hay un problema en declaracion de variables\n" 
                            cuadro_texto.insert(tk.END,f'{principal[i]["string_errores"]}')   
                            ...
                            basura.append(lista_espera[j])
                            lista_espera.remove(lista_espera[j])
                elif valor =="5": # Por prioridad Expropiativo
                    if cont2 == 0:
                        lista_espera = expropiativo 
                        cont2 += 1
                    lista_espera.sort(key=lambda x: x.prioridad, reverse=True)
                    print(lista_espera)
                    j=0
                    # Ejecutar el programa con la mayor prioridad
                    i = lista_espera[j].id
                    nombre_archivo = tk.Label(ventana, text= f'{principal[i]["nombre"]}.ch', font=("Arial", 9,"bold"), fg="white", bg="#1D09F3")
                    nombre_archivo.place(y="306", x="660")
                    if principal[i]["contador_errores"] == 0:
                ...
                elif valor =="6": # Por prioridad no Expropiativo
                        # Ordenar la lista de espera por prioridad de forma descendente
                    if cont2 == 0:
                        lista_espera = expropiativo 
                        cont2 += 1
                    lista_espera.sort(key=lambda x: x.prioridad, reverse=True)
                    # Ejecutar el programa con la mayor prioridad sin interrupciones
...
def cargar_archivo():
    ...
    create_process(array_prog, principal[programas]["inicio"]) 
    for i in procesos:
        print(i.tiempo_llegada)
...
"""
 for w in range(min(quantum, lista_espera[j].ins_num)):
                            k = my_array[inicio]
                            proceso_instruccion(i, k, acumulador, inicio)
                            my_array[0] = acumulador
                            inicio += 1
                            ventana.update()
                        lista_espera[j].ins = inicio
